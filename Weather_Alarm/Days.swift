...
class Days: UIView
{
    ...
    override init(frame: CGRect) {
       ...
       // sunButton.titleLabel?.adjustsFontSizeToFitWidth = true
        ...
        monButton.transform = CGAffineTransform(rotationAngle: ClockFace.degree2radian(a: 293))
       
        self.addSubview(monButton)
        
        //
        
        tueButton.transform = CGAffineTransform(rotationAngle: ClockFace.degree2radian(a: 313))
        ...
        thurButton.transform = CGAffineTransform(rotationAngle: ClockFace.degree2radian(a: 3))
        ...
        satButton.transform = CGAffineTransform(rotationAngle: ClockFace.degree2radian(a: 47))
    ...
    override func layoutSubviews()
    {
        ...
        sunButton.bounds = CGRect(x: 0.0, y: 0.0, width:buttonsize*1.02 , height:buttonsize*0.683)
        sunButton.center = CGPoint(x: self.bounds.size.width*0.105, y: self.bounds.size.height*0.55)
        
        //monday
      
        monButton.bounds = CGRect(x: 0.0, y: 0.0, width: buttonsize, height: buttonsize*0.68)
        monButton.center = CGPoint(x: self.bounds.size.width*0.135, y: self.bounds.size.height*0.465)
        
        //tuesday
        
        tueButton.bounds = CGRect(x: 0.0, y: 0.0, width: buttonsize, height: buttonsize*0.7)
        tueButton.center = CGPoint(x: self.bounds.size.width*0.222, y: self.bounds.size.height*0.393)
        
        //wednesday
       
        wedButton.bounds = CGRect(x: 0.0 ,y: 0.0, width: buttonsize*1.01, height: buttonsize*0.689)
        wedButton.center = CGPoint(x: self.bounds.size.width*0.363, y: self.bounds.size.height*0.3435)
        
        //thursday
        
        thurButton.bounds = CGRect(x: 0.0 ,y: 0.0 , width: buttonsize*1.045, height: buttonsize*0.69)
        thurButton.center = CGPoint(x: self.bounds.size.width*0.522, y: self.bounds.size.height*0.33)
        
        //friday
        
        friButton.bounds = CGRect(x: 0.0 ,y: 0.0 , width: buttonsize, height: buttonsize*0.69)
        friButton.center = CGPoint(x: self.bounds.size.width*0.671, y: self.bounds.size.height*0.351)
        
        //Saturday
        
        satButton.bounds = CGRect(x: 0.0 ,y: 0.0, width: buttonsize*0.99, height: buttonsize*0.7)
        satButton.center = CGPoint(x: self.bounds.size.width*0.791, y: self.bounds.size.height*0.401)
